...
void parse_args(int argc, char* argv[], char*& infile, int& minsup, bool& print, unsigned long long & init_cache_size){
...
int main(int argc, char* argv[])
{
  ...
  storage_manager<GRAPH_PAT, GRAPH_VAT, ALLOCATOR, file_storage> file_map (init_cache_size);  
  pat_fam<GRAPH_PAT>::IT cit = level_one_pats.begin();
  while(cit != level_one_pats.end()) {
    GRAPH_PAT* p = *cit;
    file_map.add_vat(p, vat_map.get_vat(p));
    cit++;
  }
  count_support<GRAPH_PR, GRAPH_MINE_PR, PAT_ST, canonical_code, ALLOCATOR, file_storage > cs(file_map);
