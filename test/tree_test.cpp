...
void print_usage(char *prog) {
  cerr<<"Usage: "<<prog<<" -i input-filename -s minsup <-I|-E> <-O|-U> [-p]"<<endl;
  ...
  cerr<<"Append -p as the last argument to print out all frequent trees to stdout"<<endl;
  exit(0);
}

void parse_args(int argc, char* argv[], char*& infile, int& minsup, bool& print, bool& induced, bool& ordered) {
  if(argc<5) {
    print_usage(argv[0]);
  }
...
int main(int argc, char* argv[]) {
  ...
  parse_args(argc, argv, infile, minsup, print, induced_f, ordered_f); // parsing the argument
