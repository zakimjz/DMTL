...
void print_usage(char *prog){
  cerr<<"Usage: "<< prog <<" -i input-filename -s minsup [-I|-E] [-p]"<<endl;
...
void parse_args(int argc, char* argv[], char*& infile, int& minsup, bool& print, bool &induced) {
  minsup = 1;
  induced = false;
  print = false;
...
void mine() {
  ...
  // COMMENT: For dealing with dataset in std::string format. 
  ...
  count_support<SEQ_PR, MINE_PROPS, PAT_ST, canonical_code, ALLOCATOR, memory_storage > cnt_sup(strg_mgr);
...
int main(int argc, char* argv[]) {
  ...
  tt_total.start();
  parse_args(argc, argv, infile, minsup, print, induced_f);
