...
void print_usage(char *prog){
  cerr<<"Usage: "<< prog <<" -i input-filename -s minsup [-p] [-f file buffer size in MB]"<<endl;
...
void parse_args(int argc, char* argv[], char*& infile, int& minsup, bool& print, unsigned long long int & init_cache_size)
{
  ...
  for (int i=1; i < argc; ++i){
    if (strcmp(argv[i], "-i") == 0){
      ...
      init_cache_size=(unsigned long long int) (atof(argv[++i])*MEG);
...
int main(int argc, char* argv[])
{
  ...
  char* infile;
  int minsup;
  unsigned long long int init_cache_size;
  
  tt_total.start();
  parse_args(argc, argv, infile, minsup, print, init_cache_size);
  ...
  storage_manager<ITEMSET, ISET_VAT, ALLOCATOR,  file_storage> strg_mgr_file(init_cache_size);
  
  pat_fam<ITEMSET>::CONST_IT cit = level_one_pats.begin();
  ...
  cout << "Size of level-1 = " << level_one_pats.size() << endl;
  
  //count_support<ISET_PR, ISET_MINE_PR, PAT_ST, canonical_code, ALLOCATOR, memory_storage > cnt_sup(strg_mgr);
  count_support<ISET_PR, ISET_MINE_PR, PAT_ST, canonical_code, ALLOCATOR, file_storage > cnt_sup(strg_mgr_file);
