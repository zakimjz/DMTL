...
void cand_gen(GRAPH_PATTERN* pat, const EDGE_MAP& emap, const int& minsup,
        const int& max_edges, pat_fam<GRAPH_PATTERN >& freq_pats,
  ...
  back_extensions(pat, emap, minsup, max_edges, freq_pats, cs);

  /// add fwd edges to get new candidates ///
  fwd_extensions(pat, emap, minsup, max_edges, freq_pats, cs);

}//end cand_gen()


template<typename PP, class MP, class PAT_ST, template<class, typename, typename, template <typename> class> class CC, template <typename> class ALLOC, class EDGE_MAP, class SM_TYPE>
void back_extensions(GRAPH_PATTERN* pat, const EDGE_MAP& emap, const int& minsup, const int& max_edges, pat_fam<GRAPH_PATTERN >& freq_pats,
                     count_support<GRAPH_PROP, V_Fk1_MINE_PROP, PAT_ST, CC, ALLOC, SM_TYPE>& cs) {
  ...
  while(rmp_it<rmp.end()-2) {
    ...
    while(lit!=lbls.end()) {
      ...
      if(cand_pat->is_valid(minsup) && cand_pat->canonical_code().size() <= max_edges) {
        //freq_pats.push_back(cand_pat);
        freq_pats_count++;
        if(print)
          cout << cand_pat;
        cand_gen(cand_pat, emap, minsup, max_edges, freq_pats, cs);
...
void fwd_extensions(GRAPH_PATTERN* pat, const EDGE_MAP& emap, const int& minsup,
        const int& max_edges, pat_fam<GRAPH_PATTERN >& freq_pats,
                    count_support<GRAPH_PROP, V_Fk1_MINE_PROP, PAT_ST, CC, ALLOC, SM_TYPE>& cs) {
#ifdef PRINT
