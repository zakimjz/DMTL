...
class vat<ISET_PROP, V_Fkk_MINE_PROP, ALLOC, ST >
{
 public:
  typedef pattern_support<V_Fkk_MINE_PROP> PAT_SUP;
  typedef vat<ISET_PROP, V_Fkk_MINE_PROP, ALLOC, ST > VAT;
  
  typedef ST<int, ALLOC<int> > IDLIST_T; // Definition of a itemset VAT.
  ...
  template<class PAT>
    ...
    if(cand_sups[0] == 0) { // No pattern_support class associated to return support value, ERROR
      ...
      cerr << "iset_vat: No candidates VATs to be generated." << endl;
      return NULL;
    }
    ...
    while(it_i!=vat_i->end() && it_j!=vat_j->end()) {
      ...
      // Execution reach is this point only if both TIDs are equal
      cand_vats[0]->push_back(*it_i);
      it_i++;
      it_j++;
    }//end while
