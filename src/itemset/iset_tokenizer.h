 ...
 */
#ifndef _ISET_TOKENIZER_H_
#define _ISET_TOKENIZER_H_
 ...
 * canocial code.
 */
template<class PP, typename MP,  typename TP, typename PAT_ST, 
...
public: 
  ...
  typedef typename ISET_PATTERN::VERTEX_T V_T;
  typedef typename ISET_PATTERN::EDGE_T E_T;
   ...
   */
  template<class SM_T>
  int parse_next_trans(std::ifstream& infile, pat_fam<ISET_PATTERN>& freq_pats, 
    ...
    while(count<num_items+3 && line<(startline+len)) {
      ...
      switch(count) {
        ...
        default:
          ...
          ISET_PATTERN* p = new ISET_PATTERN();
